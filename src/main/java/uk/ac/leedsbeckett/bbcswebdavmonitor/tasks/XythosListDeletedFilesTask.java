...
public class XythosListDeletedFilesTask extends BaseTask
{
  ...
  public void doTask() throws InterruptedException
  {
    ...
    Path logfile = bbmonitor.logbase.resolve( "deletedfiles-" + bbmonitor.dateformatforfilenames.format( new Date(System.currentTimeMillis() ) ) + ".csv" );
    ...
    try ( PrintWriter log = new PrintWriter( new FileWriter( logfile.toFile() ) ); )
    {
      ...
      for (int i = 0; i < l_fileSystems.length; ++i)
      {
        ...
        {
          bbmonitor.logger.error( "Error while running task.", ex );
        }          
        finally
        {
